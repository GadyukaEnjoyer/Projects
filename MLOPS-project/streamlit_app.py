...
st.markdown(
    "Введите текст поста — предсказание выполняется через **ClearML inference endpoint**."
)
...
if st.button("Predict", type="primary", use_container_width=True):
    ...
    if not cleaned:
        ...
        try:
            ...
            st.error(
                "Inference endpoint недоступен. "
                "Запустите: `uvicorn inference_api:app --host 127.0.0.1 --port 8000`"
            )
